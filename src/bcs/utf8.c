 ...
 */
int try_utf8_to_ascii(const uint8_t* in, size_t in_len, uint8_t* out,
                      size_t max_out_len, bool* out_is_utf8) {
    if (!in) {
        return 0;
    }
    ...
    while (i < in_len) {
        if (in[i] < 0x80) {
            ...
            if (i + 2 >= in_len || (in[i + 1] & 0xc0) != 0x80 ||
                (in[i + 2] & 0xc0) != 0x80 ||
                (in[i] == 0xe0 && (in[i + 1] & 0xe0) == 0x80) || /* overlong? */
                (in[i] == 0xed &&
                 (in[i + 1] & 0xe0) == 0xa0) || /* surrogate? */
