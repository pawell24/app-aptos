...
static void test_dynamic_bytes(void** state) {
    ...
    uint8_t raw[] = {6,    0x41, 0x70, 0x74, 0x6f, 0x73,
    ...
    assert_memory_equal(result,
                        ((uint8_t[]){0x41, 0x70, 0x74, 0x6f, 0x73, 0x41}), 6);
}

static void test_string(void** state) {
    (void)state;

    uint8_t raw[] = {19, 48, 120, 49,  58, 58,  99,  111, 105, 110,
                     58, 58, 116, 114, 97, 110, 115, 102, 101, 114};
    buffer_t buf = {.ptr = raw, .size = sizeof raw, .offset = 0};
    uint8_t str[32] = {0};
    size_t str_len = 0;
