...
static void test_transaction_utils_check_encoding(void** state) {
    ...
    const uint8_t good_ascii[] = {0x48, 0x65, 0x6c,
                                  0x6c, 0x6f, 0x21};                   // Hello!
    const uint8_t bad_ascii[] = {0x32, 0xc3, 0x97, 0x32, 0x3d, 0x34};  // 2×2=4

    assert_true(
        transaction_utils_check_encoding(good_ascii, sizeof(good_ascii)));
    assert_false(
        transaction_utils_check_encoding(bad_ascii, sizeof(bad_ascii)));
}
...
static void test_transaction_utils_strcasecmp(void** state) {
    ...
    assert_int_not_equal(
        _strcasecmp("Hello World! Hello World!", "Hello World!"), 0);
}

int main() {
    const struct CMUnitTest tests[] = {
        cmocka_unit_test(test_transaction_utils_check_encoding),
